        ...
        if 'pipeline' in st.session_state:
            ...
            if update_results:
                ...
                if 'selected_class' not in st.session_state:
                    st.session_state.selected_class = results_df['Class'].unique()[0]

                selected_class = st.selectbox('Sélectionner une classe pour afficher les résultats', results_df['Class'].unique(), index=list(results_df['Class'].unique()).index(st.session_state.selected_class))
                ...
                with tabs[1]:
                    indices_of_selected_class = [i for i, label in enumerate(y_dev) if label == selected_class]
                    ...
                    with previous_button:
                        if st.button('Précédent'):
                            st.session_state.instance_index = max(st.session_state.instance_index - 1, 0)
                    
                    with next_button:
                        if st.button('Suivant'):
                            st.session_state.instance_index = min(st.session_state.instance_index + 1, len(indices_of_selected_class) - 1)

                    instance_index = st.session_state.instance_index
                    selected_instance_index = indices_of_selected_class[instance_index]
                    st.write(f'Instance sélectionnée : {selected_instance_index}')

                    if st.button('Afficher les résultats pour l\'instance'):
                        # Accéder à l'instance spécifique dans shap_values
                        instance_shap_values = np.sum([shap_values[classe][selected_instance_index, :] for classe in range(len(shap_values))], axis=0)
                        ...
                        for i, feature_name in enumerate(X_dev.columns):
                            instance_data.append({
                                ...
                                'Instance Value': X_dev.iloc[selected_instance_index][feature_name]
                            })
